...
def fit_multiclass_rf_balanced(train_df, test_df):
    """
    Fits a multi-class logistic regression model with constituency context
    for voting behavior prediction. Classes are balanced.
    """
    # Party mapping
    party_names = {
    1: 'Labour',    # 
    ...
    random_state=42,
    class_weight='balanced'  # Fixed as balanced
)
...
# Test performance
    ...
    with pd.ExcelWriter('balanced_RF_model_results.xlsx') as writer:
    # Overall metrics
        pd.DataFrame({
        ...
        # Add comparison of top features by both methods
    ...
    print("\nResults saved to 'balanced_RF_model_results.xlsx'")
     
    cv_results = pd.DataFrame(random_search.cv_results_)
    
    
    with pd.ExcelWriter('RF_model_balanced_cv.xlsx') as writer:
